...
class CSharpToCpp(Translator):
    ...
    FIRST_RULES = [
        ...
        (r"(?P<access>(private|protected|public): )?static readonly (?P<type>[a-zA-Z0-9]+) (?P<name>[a-zA-Z0-9_]+) = new (?P=type)\(\);", r"\g<access>inline static \g<type> \g<name>;", None, 0),
        ...
        # ex.Message
        # ex.what()
        (r"(?P<scope>/\*~(?P<variable>[_a-zA-Z0-9]+)~\*/)(?P<separator>.|\n)(?P<before>((?<!/\*~(?P=variable)~\*/)(.|\n))*?)(?P=variable)\.Message", r"\g<scope>\g<separator>\g<before>\g<variable>.what()", None, 10),
        # Remove scope borders.
        # /*~ex~*/
        # 
        (r"/\*~[_a-zA-Z0-9]+~\*/", r"", None, 0),
    ...
    LAST_RULES = [
        # ICounter<int, int> c1;
        # ICounter<int, int>* c1;
        (r"(?P<abstractType>I[A-Z][a-zA-Z0-9]+(<[^>\r\n]+>)?) (?P<variable>[_a-zA-Z0-9]+);", r"\g<abstractType>* \g<variable>;", None, 0),
        # (expression)
        # expression
        ...
        # null
        # nullptr
        (r"(?P<before>\r?\n[^\"\r\n]*(\"(\\\"|[^\"\r\n])*\"[^\"\r\n]*)*)(?<=\W)null(?P<after>\W)", r"\g<before>nullptr\g<after>", None, 10),
        # default
        # 0
